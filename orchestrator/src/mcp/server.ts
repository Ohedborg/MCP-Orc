...
export function createMcpServer(repository: Repository): McpServer {
  ...
  server.tool(
    ...
    async (rawInput) => {
      ...
      const runId = randomUUID();
      const createdAt = new Date().toISOString();
      repository.insertRun({ run_id: runId, workflow_id: "build_component_workflow", status: "queued", created_at: createdAt, started_at: null, finished_at: null, error: null });
  ...
  server.tool(
    ...
    async (rawInput) => {
      ...
      if (!run) {
        log("warn", "run trace not found", { request_id: reqId, tool: "get_run_trace", run_id: input.run_id });
